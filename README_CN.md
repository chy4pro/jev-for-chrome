...
4. 页面先把目标准备好（还没过期、滚进视口、位置稳定、没被遮住），然后通过 Chrome 的 DevTools 协议派发输入，页面收到的是和鼠标键盘一样的真实点击和按键。nanobrowser、Taxy、browser-use 都是这么做的；这需要可选的 `debugger` 权限，第一次运行时申请，可在选项页关闭，没有它就退回合成 DOM 事件。点击只触发一次。不会偷偷替模型按 Enter：当聚焦的文本框里有内容时，会单独提供一个 `PRESS_ENTER` 控件，由模型自己选择（这是相对参考项目动作空间唯一的新增；arXiv、Wolfram Alpha 这类站点没有提交按钮）。
