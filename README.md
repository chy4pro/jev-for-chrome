...
4. The page prepares the target (still fresh, scrolled into view, stopped moving, not covered) and the input is dispatched through Chrome's DevTools protocol, so the page receives a real click or keystroke, as from a mouse and keyboard. That is how nanobrowser, Taxy and browser-use drive pages; it needs the optional `debugger` permission, requested on the first run and switchable in Options. Without it, synthetic DOM events are used. A click is dispatched once. Enter is never pressed implicitly: when a focused text field holds text, a separate `PRESS_ENTER` control is offered and the model has to choose it (this is the one addition to the reference action space; sites like arXiv and Wolfram Alpha have no submit button).
